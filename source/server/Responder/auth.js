...
const initFirebaseAdminApp = (firebaseAdminTokenObject) => {
  ...
  return {
    verifyIdToken: (authToken) => firebaseTokenGenerator.verifyIdToken(authToken) // async, will throw on failed verify
  }
}
...
const responderAuthVerifyToken = __DEV__
  ? async (store, firebaseAdminApp, authToken) => {
    ...
    return store.setState({ user: { id: 'DEBUG_USER_ID', name: 'DEBUG_USER_NAME', email: 'DEBUG_USER_EMAIL' } })
  }
  : async (store, firebaseAdminApp, authToken) => {
    ...
    __DEV__ && console.log('[AuthVerifyToken]', decodedToken)
    const { uid, name, email } = decodedToken
    return store.setState({ user: { id: uid, name, email } })
  }
